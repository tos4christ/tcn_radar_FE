...
class TableData extends React.Component {
  ...
  fetchReadings(dataId) {
    const url = `/current?feeder_name=${this.props.feeder_name}&current_id=${dataId}&level=330`;
    fetch(url, {
      method: 'GET',
      mode: 'no-cors',
      cache: 'no-cache',
  ...
  onChange(event) {
    const url = `/current?feeder_name=${this.props.feeder_name}&current_id=${this.props.id}&level=330`;
